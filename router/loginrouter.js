...
router.post('/', async (req, res) => {
    ...
    console.log(`inputLoginType: ${inputLoginType}`);
    ...
    if (inputLoginType === "kakao") {
        ...
        // TODO
        let googleProfileId = await googleauth(inputKey);
        console.log(`googleProfileId: ${googleProfileId}`);
        if (!googleProfileId) {
            // 카카오 토큰이 유효하지 않다.
            resultObject = createJson.error("google_auth_error");
            res.status(401).json(resultObject);
            return res.end()
        }

        inputKey = "G-" + googleProfileId;
    ...
    if (isRead) {
        ...
        try {
            ...
            console.log(`데이터베이스 result: ${result}`);
            if (result !== null) {
                let nameList = ["token", "userId"];
                let valueList = [token, userId];
                resultObject = createJson.multi(nameList, valueList);
